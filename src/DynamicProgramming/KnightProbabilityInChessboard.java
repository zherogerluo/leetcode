...
public class KnightProbabilityInChessboard {
     ...
     *
     * Typical top-down DP. Note we need to incorporate K into memoization.
     */
    class Solution1 {
        public double knightProbability(int N, int K, int r, int c) {
            return helper(N, K, r, c, new double[N*N][K+1]);
        }
        ...
        private double helper(int N, int K, int r, int c, double[][] memo) {
            ...
            if (memo[r*N+c][K] != 0) {
                return memo[r*N+c][K] - 1;
            }
            double res = 0;
            res += helper(N, K-1, r-2, c-1, memo);
            res += helper(N, K-1, r-2, c+1, memo);
            res += helper(N, K-1, r-1, c-2, memo);
            res += helper(N, K-1, r-1, c+2, memo);
            res += helper(N, K-1, r+1, c-2, memo);
            res += helper(N, K-1, r+1, c+2, memo);
            res += helper(N, K-1, r+2, c-1, memo);
            res += helper(N, K-1, r+2, c+1, memo);
            res /= 8;
            // offset to distinguish from uninitialized state if zero probability
            memo[r*N+c][K] = res + 1;
            return res;
        }
    }
